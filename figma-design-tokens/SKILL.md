...
50–950, by hue          primary/secondary/        surface/text/icon/border  paragraph-spacing
no meaning              info/success/error/...    × Light / Dark modes      × Desktop / Mobile modes
hidden from pickers     hidden from pickers        the only layer consumed   feeds bound text styles
```

The guiding rule from the source methodology: **only Mapped (and Responsive) tokens are consumed by components.** Brand and Alias are the plumbing — hide them from the property picker (empty `scopes`) so designers reach for semantic tokens.
...
7. **Responsive** — create the collection with **Desktop** + **Mobile** modes; `FLOAT` vars for `font-size` / `line-height` / `paragraph-spacing` across a type scale. Keep body (`paragraph-md`) ≥16px for accessibility.
8. **Typography styles** — add `font-family` / `font-weight` STRING vars to Brand, then create Figma text styles (Hero, H1–H6, Paragraph L/M/S, Caption) bound to the Responsive vars + font vars.
9. **Hide primitives** — set `scopes = []` on all Brand + Alias color vars so only Mapped/Responsive appear in pickers.
...
11. **Components** (the rest of the design system) — once tokens exist, build token-bound components (Button, Input, Card, Badge…) whose every fill/border/label color is a Mapped token, so they inherit Light/Dark theming for free. Full pattern, the variant-matrix recipe, and the gotchas (hug-sizing, `combineAsVariants` grid, component properties) are in **[references/components.md](references/components.md)**. Build atoms before molecules, one component at a time, screenshot to verify.
...
- Tiers: 3-tier Brand→Alias→Mapped + Responsive.
...
- Modes: Light/Dark on Mapped, Desktop/Mobile on Responsive.
- Hide Brand + Alias from pickers.
